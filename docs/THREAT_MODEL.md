...
| **T-RESOLVE-SOCK** | Attack the plain-text resolve socket's hand-rolled parser instead of the HTTP one | A1, A2 | Same three gates as `/v1/resolve` — filesystem mode, token, `SO_PEERCRED` — and the identical `Capability::Read` scope, so it grants nothing an HTTP-authenticated caller could not already reach. A fixed line-length cap and path-count cap are enforced before any store access; a per-read and a whole-connection timeout bound a stalled client. Always listening, with no separate enable/disable toggle. | The parser is new code, unlike the HTTP path where axum/hyper absorb this class of bug. A malformed request is audited as `ProtocolViolation` the same way a bad token is, so an attempt still leaves a trace even before authentication is reached. |
...
| **T-BUILTIN-UNRESTRICTED** | An operator attaches the built-in `unrestricted` policy to avoid writing a scoped one | A3 | `unrestricted` grants read, write, delete and list on `**`, but never `admin` — a token bound to it can read or change any secret but can never create, revoke or repolicy a token. Still requires generating and, ideally, uid-binding a token; there is no anonymous or tokenless path to a secret. | Functionally equivalent to T-BLAST-APP with the broadest possible scope. Intended for a first token or local experimentation, not a production service — replace it with a scoped policy once the shape of what a service actually needs is known. |
...
| A built-in policy self-heals without clobbering an operator's edit | `ensure_policy_exists_creates_but_never_overwrites`, plus an end-to-end run: delete `unrestricted` from an existing store, restart, confirm it returns; separately, `policy put` a custom `unrestricted` document, restart, confirm it survives. |
| `unrestricted` grants secret access but never administration | End-to-end: a token bound only to `unrestricted` can `put`/`get`/`rm` an arbitrary path, and is refused (`not found or not permitted`) when it attempts `token create`. |
| The resolve socket enforces the identical gates as `/v1/resolve`, including on a degenerate request | `resolve_socket_returns_every_requested_path_in_one_request`, `resolve_socket_refuses_a_token_bound_to_another_uid`, `resolve_socket_requires_a_token_even_for_an_empty_batch` and its HTTP-side counterpart `resolve_with_an_empty_batch_still_requires_a_token`, `resolve_socket_rejects_malformed_input_without_hanging` — all drive a real daemon over a real second socket. |
