...
fn a_stored_token_is_found_by_its_prefix_with_its_binding_intact() {
    ...
    store
        .put_policy(&policy("billing", "prod/billing/**"))
        .unwrap();
    ...
    assert_eq!(
        found.bound_uid,
        Some(33),
        "the uid binding must survive a round trip"
    );
...
fn attached_policies_come_back_parsed_and_ordered() {
    ...
    persist(
        &mut store,
        &issued,
        &["billing".to_owned(), "audit".to_owned()],
    )
    .unwrap();

    let attached = store.policies_for_token(issued.id).unwrap();
    assert_eq!(
        attached.iter().map(|p| p.name.as_str()).collect::<Vec<_>>(),
        ["audit", "billing"]
    );
}
...
fn naming_a_policy_that_does_not_exist_creates_no_token_at_all() {
    ...
    let outcome = persist(
        &mut store,
        &issued,
        &["billing".to_owned(), "absent".to_owned()],
    );
...
fn an_expiry_survives_a_round_trip() {
    ...
    let found = store.token_by_prefix(&issued.prefix).unwrap().unwrap();
    assert_eq!(
        found.expires_at.unwrap().unix_timestamp(),
...
fn policies_can_be_replaced_listed_and_deleted() {
    ...
    store
        .put_policy(&policy("billing", "prod/billing/readonly/**"))
        .unwrap();
    assert_eq!(
        store.list_policy_names().unwrap().len(),
        2,
...
fn ensure_policy_exists_creates_but_never_overwrites() {
    ...
    assert!(
        store
            .ensure_policy_exists(&policy("unrestricted", "**"))
            .unwrap(),
        "first call creates it"
    );
    assert!(
        !store
            .ensure_policy_exists(&policy("unrestricted", "prod/**"))
            .unwrap(),
